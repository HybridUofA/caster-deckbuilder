...
func searchTypeRank(cardType string) int {
	switch normalizeSortText(cardType) {
	case "caster":
		return 0
	case "servant":
		return 1
...
func SortForSearch(matches []Card) {
	sort.SliceStable(
		...
		func(i int, j int) bool {
			...
			leftName := normalizeSortText(left.Name)
			rightName := normalizeSortText(right.Name)
...
// normalizeSortText prepares card fields for deterministic comparisons.
func normalizeSortText(value string) string {
